    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRawat.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("bayar_piutang","?,?,?,?,?,?,?,?,?,?,?","Pembayaran",11,new String[]{
                    ...
                    sukses=jur.simpanJurnal(NoRawat.getText(),"U","K051 BAYAR PIUTANG"+", OLEH "+akses.getkode());    //tambah chan               
            }else{
                sukses=false;
            }  
        ...
        if(NoRawat.getText().trim().equals("")){
            ...
            if(tbKamar.getSelectedRow()>-1){
                ...
                if(Sequel.queryu2tf("delete from bayar_piutang where tgl_bayar=? and no_rkm_medis=? and no_rawat=? and kd_rek=? and kd_rek_kontra=?", 5,new String[]{
                    ...
                    sukses=jur.simpanJurnal(NoRawat.getText(),"U","K052 PEMBATALAN BAYAR PIUTANG"+", OLEH "+akses.getkode());     //tambah chan
                }else{
                    sukses=false;
                }
